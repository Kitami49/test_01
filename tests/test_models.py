...
def test_異常系_num1が空_Calcのバリデーション():
    with pytest.raises(ValidationError) as e:
        models = Calc(num2=2)
        print("異常系：num1=" + str(models.num1) + "、num2=" + str(models.num2))
        models.full_clean()
    assert str(e.value) == "{'num1': ['このフィールドには NULL を指定できません。']}"
    
def test_異常系_num2が空_Calcのバリデーション():
    with pytest.raises(ValidationError) as e:
        models = Calc(num1=1)
        print("異常系：num1=" + str(models.num1) + "、num2=" + str(models.num2))
        models.full_clean()
    assert str(e.value) == "{'num2': ['このフィールドには NULL を指定できません。']}"
